...
\\
\\
In choosing climate patterns as the focus of our analysis, serving as aggregators of both known and unknown \textit{Aedes}-borne disease climate drivers, we tackle the current technical challenges of climate-driven, vector-borne disease forecasting. Climate patterns provide a framework that can be used to forecast disease transmission more reliably than existing methods that use temperature and precipitation alone, which not only may not paint the full picture of climate impacts on transmission, but can also be unreliable in certain regions or timescales where skill may be lacking. The findings presented here are designed to, first, establish which variability timeframes are most important for disease prediction in a global context, and second, identify the specific climate patterns that drive these relationships.

\subsection{Analysis 1: Multi-timescale climate decomposition of $R_0$} \label{sec-discussion-analysis-1}

Climate decomposition results of Section \ref{sec-results-1}
